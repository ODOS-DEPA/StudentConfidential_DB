...
//Button to display the rows status according to the user preference
// import React, { useState } from 'react';
// import axios from 'axios';
// import { ToastContainer, toast } from 'react-toastify';
// import 'react-toastify/dist/ReactToastify.css';
...
//   const [searchTerm, setSearchTerm] = useState("");
//   const [tableType, setTableType] = useState("checkstatus");
//   const [statusFilter, setStatusFilter] = useState("all"); // filter: all | new | updated | untouched
...
//   // Safely render even if data is empty
...
//                   // Prepare exact changed fields list (avoid substring collisions)
...
//                     <tr
//                       key={i}
//                       style={{ backgroundColor: bgColor, borderBottom: '1px solid #eee', transition: 'background 0.3s' }}
//                       onMouseEnter={e => (e.currentTarget.style.backgroundColor = '#e6f7ff')}
//                       onMouseLeave={e => (e.currentTarget.style.backgroundColor = bgColor)}
//                     >
//                       {columns.map((key, j) => {
//                         const isChanged = row._status === 'updated' && changedFieldsList.includes(key);
//                         const cellStyle = {
//                           padding: '8px',
//                           textAlign: 'center',
//                           backgroundColor: isChanged ? '#fff2cc' : undefined,
...
//   // Columns for header (from the full preview set, not the filtered slice)
//   const tableColumns = previewData.length > 0 ? Object.keys(previewData[0]) : [];

//   // Pagination (safe when filteredData.length === 0)
...
//           // Normalize currentStatus column
//           if (col.toLowerCase() === 'currentstatus') {
//             const normDb = ['PASS','FAIL'].includes(String(dbVal).toUpperCase()) ? String(dbVal).toUpperCase() : null;
//             const normUp = ['PASS','FAIL'].includes(String(uploadVal).toUpperCase()) ? String(uploadVal).toUpperCase() : null;
//             return normDb !== normUp;
//           }
...
//                         // Status column display
//                         if (key.toLowerCase() === 'currentstatus') {
//                           if (value === null || value === undefined || value === '' || String(value).toLowerCase() === 'null') {
//                             displayVal = '–';
//                           } else if (value === true || value === 1 || value === '1' || String(value).toUpperCase() === 'TRUE' || value==='Pass' || value==='PASS' || value==='pass') {
//                             displayVal = 'Pass';
//                           } else if (value === false || value === 0 || value === '0' || String(value).toUpperCase() === 'FALSE' || value==='fail' || value==='FAIL' || value==='Fail') {
//                             displayVal = 'Fail';
//                           } else {
//                             displayVal = '–';
//                           }
...
//fixing the currentstatus not being updated in database
// import React, { useState } from 'react';
// import axios from 'axios';
// import { ToastContainer, toast } from 'react-toastify';
// import 'react-toastify/dist/ReactToastify.css';
...
//   // Normalize values for stage columns and currentStatus
//   const normalizeBoolean = (val, isStatus=false) => {
...
//     if (val === 1 || val === '1' || val === true || String(val).toLowerCase() === 'true') return true;
//     if (val === 0 || val === '0' || val === false || String(val).toLowerCase() === 'false') return false;
//     return null;
//   };
...
//         dbColumns.forEach(col => {
//           let value = uploadRow && uploadRow[col] !== undefined ? uploadRow[col] : dbRow[col];

//           if (col.toLowerCase().startsWith('stage') || col.toLowerCase() === 'currentstatus') {
//             mergedRow[col] = normalizeBoolean(value, col.toLowerCase() === 'currentstatus');
//           } else {
//             mergedRow[col] = value ?? null;
//           }
//         });
...
//         const changedFields = comparableCols.filter(col => {
//           if (col.toLowerCase().startsWith('stage') || col.toLowerCase() === 'currentstatus') {
//             const normDb = normalizeBoolean(dbRow[col], col.toLowerCase() === 'currentstatus');
//             const normUp = normalizeBoolean(mergedRow[col], col.toLowerCase() === 'currentstatus');
//             return normDb !== normUp;
//           }
//           return (dbRow[col] ?? null) !== (mergedRow[col] ?? null);
//         });
...
//           dbColumns.forEach(col => {
//             if (col.toLowerCase().startsWith('stage') || col.toLowerCase() === 'currentstatus') {
//               row[col] = normalizeBoolean(r[col], col.toLowerCase() === 'currentstatus');
//             } else {
//               row[col] = r[col] ?? null;
//             }
//           });
...
//                   const changedFieldsList = String(row._changedFields || "").split(',').map(s => s.trim()).filter(Boolean);
...
//                         const isStageCol = key.toLowerCase().startsWith('stage');
//                         const isStatusCol = key.toLowerCase() === 'currentstatus';
//                         let value = row[key];
//                         let displayVal = value;

//                         if (isStageCol || isStatusCol) {
//                           displayVal = value === null ? '–' : value ? '✅' : '❌';
//                         }
...
//adjust the code to be compatible with the new database
// import React, { useState } from 'react';
// import axios from 'axios';
// import { ToastContainer, toast } from 'react-toastify';
// import 'react-toastify/dist/ReactToastify.css';
...
//                         // Handle currentStatus as boolean
//                         if (keyLower === 'currentstatus') {
//                           if (value === true || value === 1 || value === '1' || String(value).toUpperCase() === 'TRUE') {
//                             displayVal = 'Pass';
//                           } else if (value === false || value === 0 || value === '0' || String(value).toUpperCase() === 'FALSE') {
//                             displayVal = 'Fail';
//                           } else {
...
//                         // Handle stage as numeric status
//                         if (keyLower.startsWith('stage') && keyLower !== 'currentstatus') {
//                           if (value === null || value === undefined || value === '' || String(value).toLowerCase() === 'null') {
//                             displayVal = '–';
//                           } else if (value === 1 || value === '1' || String(value).trim() === 'ผ่าน') {
//                             displayVal = '✅';
//                           } else if (value === 0 || value === '0' || String(value).trim() === 'ไม่ผ่าน') {
//                             displayVal = '❌';
//                           } else {
//                             displayVal = '–';
//                           }
...
//                         // // Handle currentStatus as string status
//                         // if (keyLower === 'currentstatus') {
//                         //   const val = value ? String(value).toUpperCase() : null;
//                         //   displayVal = val === 'PASS' ? 'Pass' : val === 'FAIL' ? 'Fail' : '–';
//                         // }
...
//readjust the logic of currentstatus to indicates which stage the student is in
// import React, { useState } from 'react';
// import axios from 'axios';
// import { ToastContainer, toast } from 'react-toastify';
// import 'react-toastify/dist/ReactToastify.css';
...
//   const calculateCurrentStatus = (row) => {
//     for (let i = 1; i <= 8; i++) {
//       const stageKey = `stage${i}`;
//       const stageVal = row[stageKey];
//       if (stageVal === null || stageVal === undefined || stageVal === '' || String(stageVal).toLowerCase() === 'null') {
//         return i; // current pending stage
//       }
//       const valStr = String(stageVal).trim();
//       if (valStr === 'ไม่ผ่าน' || valStr === '0') {
//         return i; // current failed stage
//       }
//     }
//     return 9; // all passed
//   };
...
//         uploadRows = lines.slice(1).map(line => {
...
//         });
//       } else {
//         uploadRows = await readExcelFile(file);
//       }

//       // Calculate currentStatus automatically
//       uploadRows = uploadRows.map(row => ({
//         ...row,
//         currentStatus: calculateCurrentStatus(row),
//       }));

//       const dbUrl = tableType === "checkstatus" ? `${domain}/students/all` : `${domain}/citizenID/all`;
//       const idKey = "StudentID";

//       const dbRes = await axios.get(dbUrl);
//       const dbRows = dbRes.data?.rows || dbRes.data || [];
//       const dbMap = new Map(dbRows.map(r => [r[idKey], r]));
//       const dbColumns = dbRows.length > 0 ? Object.keys(dbRows[0]) : Object.keys(uploadRows[0]);
...
//           const uploadVal = mergedRow[col];
//           return dbVal != uploadVal;
//         });

//         return { ...mergedRow, _status: changedFields.length > 0 ? 'updated' : 'untouched', _changedFields: changedFields.join(', ') };
//       });

//       // Add new rows
//       const newRows = uploadRows.filter(r => !dbMap.has(r[idKey])).map(r => ({ ...r, _status: 'new', _changedFields: '' }));
...
//     setConfirming(true);
//     try {
//       const confirmUrl = tableType === "checkstatus"
//         ? `${domain}/DataUpload/confirmUpload`
//         : `${domain}/citizenID/upload/confirmUpload`;
...
//                 {data.map((row, i) => {
//                   const status = row._status;
//                   const bgColor = status === 'new' ? '#e6ffe6' :
//                     status === 'updated' ? '#fffbe6' :
//                       status === 'untouched' ? '#f0f0f0' :
//                         i % 2 === 0 ? '#fff' : '#f9f9f9';
...
//                         // Stage columns
//                         if (keyLower.startsWith('stage') && keyLower !== 'currentstatus') {
//                           if (value === null || value === undefined || value === '' || String(value).toLowerCase() === 'null') displayVal = '–';
//                           else if (String(value).trim() === 'ผ่าน' || value === 1 || value === '1') displayVal = '✅';
//                           else if (String(value).trim() === 'ไม่ผ่าน' || value === 0 || value === '0') displayVal = '❌';
//                           else if (String(value).trim() === 'รอ' || String(value).trim() === 'รอดำเนินการ' || String(value).trim() === 'pending' || String(value).trim() === 'Pending') displayVal = '⏳';
//                           else displayVal = '–';
//                         }
...
//                           textAlign: 'center',
//                           backgroundColor: isChanged ? '#fff2cc' : undefined,
//                           fontWeight: (isChanged || displayVal === row.currentStatus) ? 'bold' : undefined,
//                           border: '1px solid #eee'
//                         };
...
//     if (searchTerm.trim()) {
//       const lowerSearch = searchTerm.toLowerCase();
//       const matchesSearch = Object.values(row).some(val => val != null && String(val).toLowerCase().includes(lowerSearch));
//       if (!matchesSearch) return false;
//     }
//     if (statusFilter !== "all" && row._status !== statusFilter) return false;
//     return true;
...
//   // Modified currentStatus calculation
//   const calculateCurrentStatus = (row) => {
//     for (let i = 1; i <= 8; i++) {
//       const stageVal = row[`stage${i}`]?.toString().trim();
//       if (!stageVal || stageVal === 'รอดำเนินการ') {
//         return i; // first pending stage
//       }
//     }
//     return 9; // all stages passed
//   };
...
//                           else if (String(value).trim() === 'ผ่าน' || value === 1 || value === '1') displayVal = '✅';
//                           else if (String(value).trim() === 'ไม่ผ่าน' || value === 0 || value === '0') displayVal = '❌';
//                           else if (String(value).trim() === 'รอดำเนินการ' || String(value).trim() === 'รอ' || String(value).trim() === 'pending'||String(value).trim() === 'Pending') displayVal = '⏳';
//                           else displayVal = '–';
//                         }
...
//fix currentstatus from ignoring fail and include pending instead
// import React, { useState } from 'react';
// import axios from 'axios';
// import { ToastContainer, toast } from 'react-toastify';
// import 'react-toastify/dist/ReactToastify.css';
...
//   // FIXED calculateCurrentStatus: 1-8 only
//   const calculateCurrentStatus = (row) => {
//     for (let i = 1; i <= 8; i++) {
//       const stageKey = `stage${i}`;
//       const val = row[stageKey];
//       if (val === null || val === undefined || val === '' || val === 'รอดำเนินการ') {
//         return i; // first pending stage
//       }
//       if (val === 'ไม่ผ่าน') {
//         return i; // failed stage, stop here
//       }
//     }
//     return 8; // all stages passed
//   };
...
//       // Calculate currentStatus for preview
//       const uploadWithStatus = uploadRows.map(r => ({
//         ...r,
//         currentStatus: calculateCurrentStatus(r),
//       }));
...
//       // Merge DB + Upload for preview
//       const previewWithStatus = dbRows.map(dbRow => {
//         const uploadRow = uploadWithStatus.find(r => r[idKey] === dbRow[idKey]);
//         const mergedRow = {};
//         dbColumns.forEach(col => {
//           mergedRow[col] = uploadRow && uploadRow[col] !== undefined ? uploadRow[col] : dbRow[col];
//         });
...
//         const comparableCols = dbColumns.filter(col => col !== idKey);
//         const changedFields = comparableCols.filter(col => mergedRow[col] != dbRow[col]);

//         return { ...mergedRow, _status: changedFields.length > 0 ? "updated" : "untouched", _changedFields: changedFields.join(", ") };
//       });

//       // Add new rows
//       const newRows = uploadWithStatus.filter(r => !dbMap.has(r[idKey])).map(r => ({
//         ...r,
//         _status: "new",
//         _changedFields: ""
//       }));
...
//                         if (keyLower.startsWith('stage')) {
//                           if (value === null || value === undefined || value === '') displayVal = '–';
//                           else if (value === 'ผ่าน' || value === 1) displayVal = '✅';
//                           else if (value === 'ไม่ผ่าน' || value === 0) displayVal = '❌';
//                           else if (String(value).trim() === 'รอดำเนินการ' || String(value).trim() === 'รอ' || String(value).trim() === 'pending'||String(value).trim() === 'Pending') displayVal = '⏳';
//                           else displayVal = '–';
//                         }
...
//latest version on changing currentstatus
// import React, { useState } from 'react';
// import axios from 'axios';
// import { ToastContainer, toast } from 'react-toastify';
// import 'react-toastify/dist/ReactToastify.css';
...
//   // Calculate currentStatus: 1-8 only
//   const calculateCurrentStatus = (row) => {
//     for (let i = 1; i <= 8; i++) {
//       const stageKey = `stage${i}`;
//       const val = row[stageKey];
...
//       // ✅ Send previewData (with currentStatus included)
//       await axios.post(confirmUrl, { confirm: true });
...
//                           } else if (value === 0 || value === '0' || String(value).trim() === 'ไม่ผ่าน') {
//                             displayVal = '❌';
//                           } else if (String(value).trim() === 'รอดำเนินการ' || String(value).trim() === 'รอ' || String(value).trim() === 'pending' || String(value).trim() === 'Pending') {
//                             displayVal = '⚠️';
//                           } else {
//                             displayVal = '–';
//                           }
...
//code from P'Santy modification
