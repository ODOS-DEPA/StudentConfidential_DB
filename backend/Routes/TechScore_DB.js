...
let DataReader; 
const TechScoreTest_DB = express.Router();
const upload = multer({ storage: multer.memoryStorage() }); // เก็บไฟล์ใน RAM (buffer)

TechScoreTest_DB.post('/', upload.single('file'), async (req, res) => {
  try {
    if (!req.file) {
...
TechScoreTest_DB.post('/confirmUpload', async (req, res) => {
  ...
  try {
    ...
    const [existingRows] = await sql.query(
      `SELECT StudentID FROM TechScoreTest WHERE StudentID IN (?)`,
      [allStudentIDs]
    );
    ...
    for (let i = 0; i < insertRows.length; i += batchSize) {
      ...
      const sqlQuery = `INSERT IGNORE INTO TechScoreTest (${columns.join(',')}) VALUES ${allValues.map(() => `(${placeholders})`).join(',')}`;
...
export default TechScoreTest_DB;
